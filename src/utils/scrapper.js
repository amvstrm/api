...
import { generateEncryptAjaxParameters, decryptEncryptAjaxResponse } from './getm3u8.js'
...
const animixurl = "https://animixplay.to"

const animixinfo = async (id, dub) => {
  if (!id.startsWith('http')) id = `${animixurl}${dub ? `${id}-dub` : id}`;
    const animeInfo = {
      id: id.split(animixurl)[1],
      title: '',
    };
    try {
...
const dllink = async (id) => {
  ...
  $('div#wrapper_bg').each((index, element) => {
    ...
    const download = $element.find('div.anime_video_body div.anime_video_body_cate div.favorites_book ul li a').attr('href');
    promises.push({
      id: id ? id : null,
...
const animeInfo = async (id) => {
  ...
  $('div#wrapper_bg').each((index, element) => {
    ...
    const synopsis = $element.find('div.anime_info_body_bg p.type').eq(1).text().replace('Plot Summary:', '').trim();
